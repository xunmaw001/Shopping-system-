	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussgouwuzixun`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(this.ruleForm.id){
					await this.$api.update(`discussgouwuzixun`, this.ruleForm);
				}else{
					await this.$api.add(`discussgouwuzixun`, this.ruleForm);
				}
				uni.setStorageSync('discussgouwuzixunCleanType',true);
